const SpeakJS = {
  ...
  layoutDiv: (cfg) => {
    ...
    SpeakJS.requestAPI(cfg.api, function(data) {
      $(el).find('.loading-wrap').remove();
      let array = data.data.items;
      array.forEach((item, i) => {
        if (cfg.pageSize && i >= cfg.pageSize) {
          return;
        }
        let date = new Date(item.createdAt)
        var cell = '<div class="timenode" index="' + i + '">';
        cell += '<div class="header"><p>' + date.toLocaleString() + '</p>';
        cell += '</div>';
        cell += '<div class="body">';
...
$(function () {
  ...
  for (var i = 0; i < els.length; i++) {
    ...
    cfg.el = el;
    cfg.api = api;
    cfg.pageSize = el.getAttribute('pageSize');
